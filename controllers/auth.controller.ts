...
class AuthController {
   ...
   public async register(req: Request, res: Response) {
      try {
         ...
         const __RToken__ = jwt.sign({ _id: email.toLowerCase().trim() }, env.SECRET_TOKEN);
   ...
   public async forgetPassword_verifyMail(req: Request, res: Response) {
      try {
         ...
         } while (user.rest_password_code === code && (code < 10000 && code > 99999));
   ...
   public async forgetPassword_verifyCode(req: Request, res: Response) {
      try {
         ...
         console.log('code', code, codeToExist.rest_password_code)
         if (
   ...
   public async forgetPassword_change(req: Request, res: Response) {
      try {
         ...
         if (qNewPassword)
            await authMailer.sendMailToSuccessChangePassword(email);

         return res.status(201).send({
            qNewPassword,
            message: 'Votre mot de passe à été modifer avec success.',
         });
      } catch (error) {
         return serverError.catchError(res, error);
      }
